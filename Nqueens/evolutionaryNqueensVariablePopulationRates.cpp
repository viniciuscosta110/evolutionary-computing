...
int hamming_distance(int *cromossome1, int *cromossome2) {
    int distance = 0;
    for (int i = 0; i < TABLE; i++) {
        if (cromossome1[i] != cromossome2[i]) {
            distance++;
        }
    }
    return distance;
...
double average_pairwise_distance(vector <Individual> population) {
    double sum = 0;
    int count = 0;

    for (int i = 0; i < POPULATION; i++) {
        for (int j = i + 1; j < POPULATION; j++) {
            int distance = hamming_distance(population[i].cromossome, population[j].cromossome);
            sum += distance;
            count++;
        }
    }
...
int main() {
    ...
    while(TOTAL_GENERATIONS > currentGeneration) {
        ...
        double currentDiversity = average_pairwise_distance(population);

        if(currentDiversity < DIVERSITY_TARGET){
            MUTATION_RATE += 0.001;
            CROSSOVER_RATE -= 0.001;
        } else if(currentDiversity >= DIVERSITY_TARGET) {
            MUTATION_RATE -= 0.001;
            CROSSOVER_RATE += 0.001;
        }
    ...
    cout << "Diversity: " << average_pairwise_distance(population) / TABLE*0.1 << endl;
